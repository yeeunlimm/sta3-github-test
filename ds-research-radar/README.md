...
- 1차 출처를 우선합니다.
- 같은 논문을 다시 분석할 때는 그 논문 페이지와 보고서만 갱신하고, 식별자가 다른 새 논문은 새 하위페이지로 추가합니다.
...
주간 흐름은 `Language / Climate / Industry / Career` 조사 담당 4명이 동시에 후보를 만들고, 별도 검토 담당이 중복·날짜·원문 일치를 확인한 뒤 Slack 발송 담당에게 넘기는 방식입니다. 논문은 **한 논문당 담당 에이전트 1명**이 원문을 분석해 모든 논문에 동일한 9개 항목 보고서를 완성합니다. 자세한 역할은 [subagent-workflow.md](references/subagent-workflow.md)를 참고합니다.
